...
Para alguns exemplos, consulte [defaults.lua](https://github.com/overextended/ox_target/blob/main/client/defaults.lua) ou [debug.lua](https://github.com/overextended/ox_target/blob/main/client/debug.lua).
...
- state: `boolean`
  - Defina `state` como `true` para desativar o targeting (o "olho") se estiver ativo, e impedir que reabra até que `state` seja `false`.
...
- parameters: `table`
  ...
  - debug?: `boolean`
  - drawSprite?: `boolean`
    - Desenha um sprite no centróide da zona. Padrão: `true`.
  - options: `TargetOptions`
