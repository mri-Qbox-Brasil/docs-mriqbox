...
```mermaid
graph TD
    A["📁 resources<br/>(Pasta principal da base)"] --> B["📁 [addons]<br/>(Arquivos addons)"]
    B --> C["📁 [veiculos]<br/>(Veículos addons)"]
    C --> D["📁 BMW55i<br/>(Pasta do veículo)"]
    D --> E["📁 stream<br/>.yft, .ytd, .ydr, etc."]
    D --> F["📁 data<br/>handling.meta, vehicles.meta, carcols.meta"]
    D --> G["📄 fxmanifest.lua<br/>Inicia (starta) o recurso"]
```
